 ...
 * @LastEditTime: 2021-08-03 06:51:15
 * @LastEditors: Please set LastEditors
 * @Description: 自定义 Promise 函数模块: IIFE
 * @FilePath: /vite/vite-project/src/views/Promise/lib/3.3.promise_then方法实现2.js
 */
(function(window) {
  ...
  Promise.prototype.then = function (onResolved, onRejected) {
    ...
    return new Promise((resolve, reject) => {
      if (self.status === PENDING) {
        ...
        setTimeout(() => {
          // 如果抛出异常，return 的 promise 就会失败，reason 就是 error
          // 如果回调函数返回不是 promise，return 的 promise 就会失败，reason 就是返回的值
          // 如果回调函数返回是 promise，return 的 promise 结果就是这个 promise 的结果
          try {
